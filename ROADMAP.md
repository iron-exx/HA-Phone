...
**Update (2026-07-28), vermutlich derselbe Bug wie oben:** Interne Anrufe (Nebenstelle -> Nebenstelle, z.B. 11 -> 15 auf dem Gigaset N510) zeigten "Anonymous", obwohl D15 (`trust_id_outbound`) laengst behoben war. Live per SIP-Trace verifiziert (AMI-Originate mit injizierter CallerID, PJSIP-Logger mitgeschnitten): der `From`-Header war korrekt (`"sandro" <sip:11@192.168.7.10>`), *kein* Anonymous auf Asterisk-Seite. Es fehlten aber `P-Asserted-Identity`/`Remote-Party-ID` komplett (`send_pai`/`send_rpid` waren fuer `pjsip_extensions.conf.j2` nie gesetzt, nur fuer den Trunk). Gigaset-DECT-Basen zeigen den Namen vermutlich nur an, wenn einer dieser Header mitkommt, statt dem blossen `From` zu vertrauen. Fix angewendet: `send_pai = yes` + `send_rpid = yes` auch auf den Extension-Endpoints (analog zum Trunk). Config-seitig geladen und per `pjsip show endpoint` bestaetigt (`send_pai: true`), ABER: im Test-INVITE (simulierter Anruf via Local-Channel + AMI-Originate, da niemand zuhause war um ein echtes Gigaset-Handset zu testen) tauchte trotzdem kein `P-Asserted-Identity`-Header auf - Ursache unklar, moeglicherweise eine zusaetzliche interne Asterisk-Bedingung (`ast_sip_can_present_connected_id`, laut Community-Threads nicht rein `send_pai`-abhaengig) oder eine Einschraenkung der Local-Channel-Simulation selbst. Naechster Schritt: echten Anruf zwischen zwei Nebenstellen mit PJSIP-Logger mitschneiden, sobald jemand vor Ort ist, um zu bestaetigen ob PAI jetzt tatsaechlich rausgeht und ob das Gigaset-Display den Namen zeigt.

**Update (2026-07-30), Ursache fuer den urspruenglichen externen Fall (2026-07-06) gefunden und behoben:** Echter externer Anruf live mitgeschnitten (SIP-Trace + `core show channel` waehrend des klingelnden Calls). Eingehende INVITE von aarenet/AareSwitch/DG trug die Rufnummer nur im blanken `From`-Header (`From: <sip:01741814689@telekom.de>`), *kein* PAI, *kein* RPID. `core show channel` auf dem `PJSIP/trunk-endpoint`-Kanal waehrend des Anrufs zeigte `Caller ID: (N/A)` - die Nummer wurde also nie ins System uebernommen, nicht erst beim Weiterleiten verworfen. Root Cause im Asterisk-Quellcode gefunden (`res/res_pjsip.c`, Funktion `ast_sip_set_id_from_invite`): bei `trust_id_inbound=yes` OHNE PAI/RPID faellt Asterisk NICHT auf den `From`-Header zurueck, sondern auf die statisch konfigurierte `callerid` des Endpoints - der `From`-Header wird nur ueberhaupt angeschaut, wenn diese statische CallerID selbst leer ist. `trunk-endpoint` hatte eine statische `callerid` fuer ausgehendes CLIP konfiguriert, wodurch die echte Anrufernummer bei jedem PAI/RPID-losen eingehenden Anruf verworfen wurde. Fix: statische `callerid` aus `[trunk-endpoint]` entfernt - ausgehend wird die CLIP-Nummer bereits explizit per `Set(CALLERID(all)=...)` im Dialplan gesetzt (Multi-DID-Feature), die statische Endpoint-CallerID war dafuer laengst redundant. Noch nicht mit einem finalen echten Anruf NACH diesem Fix verifiziert (der auswertete Trace war der Anruf, der den Bug aufgedeckt hat, nicht mehr getestet mit der Korrektur) -> naechster Schritt: ein weiterer echter externer Testanruf zur Bestaetigung.
