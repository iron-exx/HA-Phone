...
case "$(echo "$ADMIN_PASSWORD" | tr '[:upper:]' '[:lower:]')" in
    changeme|admin)
        bashio::log.warning "admin_password is a default password — the web UI forces a password change on first login (Passwort ändern erforderlich)."
        ;;
esac
...
if [ -n "$EXTERNAL_IP" ]; then
    ...
    bashio::log.warning "ha-phone: could not detect external IP — Asterisk starts in LAN-only mode."
fi
if [ ! -f /data/asterisk/tls/asterisk.crt ] || [ ! -f /data/asterisk/tls/asterisk.key ]; then
    bashio::log.warning "ha-phone: TLS cert missing -- [transport-tls] not written, TLS test extension unreachable."
fi

# pjsip_local.conf (transport NAT settings + [transport-tls], D-02/D-06) is rendered
# from backend/conf_templates/pjsip_local.conf.j2 — the SAME template the web UI's
# public-IP setting uses, so saving the IP there can no longer drop [transport-tls].
# An IPv6 external address goes only onto [transport-udp-ipv6].
PYTHONPATH=/app python3 -c "
import os, sys
os.environ['BPX_DATA_DIR'] = '/data'
from backend.pjsip_local import write_pjsip_local
try:
    write_pjsip_local(sys.argv[1])
except ValueError:
    write_pjsip_local('')
print('pjsip_local.conf written')
" "$EXTERNAL_IP" || {
    bashio::log.warning "ha-phone: pjsip_local.conf render failed — writing LAN-only placeholder."
    echo "; pjsip_local.conf render failed — LAN-only mode" > "$PJSIP_LOCAL"
}
